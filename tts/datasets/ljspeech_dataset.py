        ...
        for wav_dir in tqdm(
            ...
            with trans_path.open() as f:
                for i, line in enumerate(f):
                    ...
                    if w_text.isascii():
                        alignment_path = wav_path.parent.parent / "alignments"
                        alignment_path /= str(i - 1) + '.npy'
                        mel_path = wav_path.parent.parent / "mels" / (f'ljspeech-mel-{i:05}.npy')
                        pitch_path = wav_path.parent / (str(i) + '-pitch.npy') 
                        energy_path = wav_path.parent / (str(i) + '-energy.npy') 
                        index.append(
                            {
