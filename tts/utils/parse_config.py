...
from tts import text_encoder as text_encoder_module
from tts.base.base_text_encoder import BaseTextEncoder
from tts.logger import setup_logging
from tts.text_encoder import FastSpeechEncoder
...
class ConfigParser:
    ...
    def get_text_encoder(self, datasets=None) -> BaseTextEncoder:
        if self._text_encoder is None:
            if "text_encoder" not in self._config:
                self._text_encoder = FastSpeechEncoder()
            else:
    ...
    def get_default_configs(cls):
        config_path = ROOT_PATH / "tts" / "config.json"
        with config_path.open() as f:
            return cls(json.load(f))

    @classmethod
    def get_test_configs(cls):
        config_path = ROOT_PATH / "tts" / "tests" / "config.json"
        with config_path.open() as f:
            return cls(json.load(f))
